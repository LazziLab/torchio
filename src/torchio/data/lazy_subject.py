...
class Lazy_Subject(dict):
    ...
    Example:
        ...
        ...     one_image=tio.Lazy_ScalarImage('path_to_image.nii.gz'),
        ...     a_segmentation=tio.Lazy_LabelMap('path_to_seg.nii.gz'),
        ...     age=45,
        ...     name='John Doe',
        ...
        >>> subject_dict = {
        ...     'one image': tio.Lazy_ScalarImage('path_to_image.nii.gz'),
        ...     'a segmentation': tio.Lazy_LabelMap('path_to_seg.nii.gz'),
        ...     'age': 45,
        ...     'name': 'John Doe',
        ...     'hospital': 'Hospital Juan Negrín',
        ... }
        >>> subject = tio.Lazy_Subject(subject_dict)
    """  # noqa: B950
    ...
    @staticmethod
    def _parse_images(images: List[Lazy_Image]) -> None:
    ...
    def check_consistent_attribute(
        ...
        Example:
            ...
            >>> subject = tio.Lazy_Subject(
            ...   image = tio.Lazy_ScalarImage(tensor=scalars, affine=af1),
            ...   mask = tio.Lazy_LabelMap(tensor=mask, affine=af2)
            ... )
    ...
    def get_torchio_subject(self) -> Subject:
        images = {}
        for image_name, image in self.items():
            if isinstance(image, Lazy_Image):
                images[image_name] = image.get_torchio_image()
    ...
    def get_images_dict(
        ...
        images = {}
        for image_name, image in self.items():
            if not isinstance(image, Lazy_Image):
                continue
    ...
    def get_first_image(self) -> Lazy_Image:
    ...
    def add_image(self, image: Lazy_Image, image_name: str) -> None:
        """Add an image to the subject instance."""
        if not isinstance(image, Lazy_Image):
            message = (
                'Lazy_Image must be an instance of lazy_image.Lazy_Image,'
...
def _subject_copy_helper(
    old_obj: Lazy_Subject,
    new_subj_cls: Callable[[Dict[str, Any]], Lazy_Subject],
):
    result_dict = {}
    for key, value in old_obj.items():
        if isinstance(value, Lazy_Image):
...
class _RawSubjectCopySubject(Lazy_Subject):
    def __copy__(self):
        return _subject_copy_helper(self, Lazy_Subject)
