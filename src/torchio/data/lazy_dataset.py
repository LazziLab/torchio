...
class Lazy_SubjectsDataset(Dataset):
    ...
    augmentation. It receives a list of instances of :class:`~torchio.Lazy_Subject`
    and an optional transform applied to the volumes after loading.

    Args:
        subjects: List of instances of :class:`~torchio.Lazy_Subject`.
    ...
    Example:
        ...
        >>> subjects_dataset = tio.Lazy_SubjectsDataset(subjects_list, transform=transform)
    ...
    def __init__(
        self,
        subjects: Sequence[Lazy_Subject],
    ...
    def __getitem__(self, index: int) -> Lazy_Subject:
        try:
            index = int(index)
        ...
        # lazy dataset doesn't apply transforms, only lazy sampler applies lazy cropping
    ...
    @classmethod
    def from_batch(cls, batch: dict) -> Lazy_SubjectsDataset:
        ...
        Args:
            batch: Dictionary generated by a data loader, containing data that
                can be converted to instances of :class:`~.torchio.Lazy_Subject`.
        """
        subjects: List[Lazy_Subject] = get_subjects_from_batch(batch)
        return cls(subjects)
    ...
    @staticmethod
    def _parse_subjects_list(subjects_list: Iterable[Lazy_Subject]) -> None:
        # Check that it's an iterable
        try:
            iter(subjects_list)
        except TypeError as e:
            message = f'Lazy_Subject list must be an iterable, not {type(subjects_list)}'
            raise TypeError(message) from e

        # Check that it's not empty
        if not subjects_list:
            raise ValueError('Lazy_Subjects list is empty')

        # Check each element
        for subject in subjects_list:
            if not isinstance(subject, Lazy_Subject):
                message = (
                    'Lazy_Subjects list must contain instances of lazy_subject.Lazy_Subject,'
                    f' not "{type(subject)}"'
                )
