...
from .io import read_affine # could update for lazy_zarr
from .io import read_image
from .io import read_shape # could update for lazy_zarr
...
class Lazy_Image(dict):
    ...
    Example:
        ...
        >>> import numpy as np
        >>> image = tio.Lazy_ScalarImage('t1.nii.gz')  # subclass of Lazy_Image
        >>> image  # not loaded yet
        Lazy_ScalarImage(path: t1.nii.gz; type: intensity)
        >>> times_two = 2 * image.data  # data is loaded and cached here
        >>> image
        Lazy_ScalarImage(shape: (1, 256, 256, 176); spacing: (1.00, 1.00, 1.00); orientation: PIR+; memory: 44.0 MiB; type: intensity)
        >>> image.save('doubled_image.nii.gz')
        >>> def numpy_reader(path):
        ...     data = np.load(path).as_type(np.float32)
        ...     affine = np.eye(4)
        ...     return data, affine
        >>> image = tio.Lazy_ScalarImage('t1.npy', reader=numpy_reader)
        ...
        **kwargs: Dict[str, Any],
        ...
        if type is None:
            warnings.warn(
                ...
                ' tio.Lazy_ScalarImage or tio.Lazy_LabelMap instead',
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def __repr__(self):
        ...
        if self._loaded:
            properties.append(f'dtype: {self.data.dtype}')
    ...
    @property
    def tensor(self, path: TypePath) -> TypeDataAffine:
        # Make sure the data type is compatible with PyTorch
        if isinstance(self.data, torch.Tensor):
            return self.data
        if isinstance(self.data, np.ndarray) or isinstance(self.data, lazy_ops.lazy_loading.DatasetView):
            tensor = check_uint_to_int(self.data)
            tensor = self._parse_tensor_shape(tensor)
            tensor = self._parse_tensor(tensor, hard_convert=True)
            affine = self._parse_affine(affine)
    ...
    def itemsize(self):
        ...
        elif isinstance(self.data, lazy_ops.lazy_loading.DatasetView):
    ...
    def get_torchio_image(self) -> Image:
        return Image(path=self.path, tensor=self.data, affine=self.affine, type=self.type)
    ...
    def _parse_tensor(
        ...
        if isinstance(tensor, np.ndarray) or (hard_convert and isinstance(tensor, lazy_ops.lazy_loading.DatasetView)):
            tensor = check_uint_to_int(tensor)
            tensor = torch.as_tensor(tensor)
        elif (not hard_convert) and isinstance(tensor, lazy_ops.lazy_loading.DatasetView):
            tensor = tensor
    ...
    def load(self) -> None:
        ...
        elif isinstance(tensors[0], lazy_ops.lazy_loading.DatasetView):
            tensor=tensors[0]
            # TODO concatenate DatasetView arrays
            # for subtensor in tensors[1:]:
    ...
    def from_sitk(cls, sitk_image):
        ...
        Example:
            ...
            >>> tio.Lazy_LabelMap.from_sitk(sitk_image)
            Lazy_LabelMap(shape: (1, 20, 30, 40); spacing: (1.00, 1.00, 1.00); orientation: LPS+; memory: 93.8 KiB; dtype: torch.IntTensor)
            >>> sitk_image = sitk.Image((224, 224), sitk.sitkVectorFloat32, 3)
            >>> tio.Lazy_ScalarImage.from_sitk(sitk_image)
            Lazy_ScalarImage(shape: (3, 224, 224, 1); spacing: (1.00, 1.00, 1.00); orientation: LPS+; memory: 588.0 KiB; dtype: torch.FloatTensor)
        """  # noqa: B950
    ...
    def as_pil(self, transpose=True):
        ...
        if transpose:
            ...
            elif isinstance(tensor, lazy_ops.lazy_loading.DatasetView):
                tensor = tensor.lazy_transpose([3, 2, 1, 0])
        else:
            if isinstance(tensor, torch.Tensor):
                tensor = tensor.permute(3, 1, 2, 0)
            elif isinstance(tensor, lazy_ops.lazy_loading.DatasetView):
                tensor = tensor.lazy_transpose([3, 1, 2, 0])
        # array = tensor.clamp(0, 255).numpy()[0].astype(np.uint8)
        array = np.clip(np.asarray(tensor[0]), 0, 255).astype(np.uint8)
        return ImagePIL.fromarray(array)
    ...
    def show(self, viewer_path: Optional[TypePath] = None) -> None:
        ...
        if self.__class__.__name__ == 'Lazy_LabelMap':
...
class Lazy_ScalarImage(Lazy_Image):
    ...
    Example:
        ...
        >>> import torchio as tio
        >>> # Loading from a file
        >>> t1_image = tio.Lazy_ScalarImage('t1.nii.gz')
        >>> dmri = tio.Lazy_ScalarImage(tensor=torch.rand(32, 128, 128, 88))
        >>> image = tio.Lazy_ScalarImage('safe_image.nrrd', check_nans=False)
    ...
    See :class:`~torchio.Lazy_Image` for more information.
    """

    def __init__(self, *args, **kwargs):
        if 'type' in kwargs and kwargs['type'] != INTENSITY:
            raise ValueError('Type of Lazy_ScalarImage is always torchio.INTENSITY')
    ...
    def hist(self, **kwargs) -> None:
        ...
        if isinstance(self.data, torch.Tensor):
            ...
            x = np.asarray(self.data.view(self.data.size))
        plot_histogram(x, **kwargs)


class Lazy_LabelMap(Lazy_Image):
    """Image whose pixel values represent categorical labels.

    Example:
        >>> import torch
        >>> import torchio as tio
        >>> labels = tio.Lazy_LabelMap(tensor=torch.rand(1, 128, 128, 68) > 0.5)
        >>> labels = tio.Lazy_LabelMap('t1_seg.nii.gz')  # loading from a file
        >>> tpm = tio.Lazy_LabelMap(                     # loading from files
        ...     'gray_matter.nii.gz',
        ...     'white_matter.nii.gz',
        ...     'csf.nii.gz',
        ... )
    ...
    def __init__(self, *args, **kwargs):
        if 'type' in kwargs and kwargs['type'] != LABEL:
            raise ValueError('Type of Lazy_LabelMap is always torchio.LABEL')
    ...
    def count_labels(self) -> Dict[int, int]:
        ...
        img_size = torch.prod(torch.tensor(self.data.shape))
        if isinstance(self.data, torch.Tensor):
            values_list = self.data.flatten().tolist()
        elif isinstance(self.data, lazy_ops.lazy_loading.DatasetView):
            values_list = list(self.data.view(self.data.size))
...
def ensure_4d(tensor: TypeData, num_spatial_dims=None) -> torch.Tensor:
    ...
    if num_dimensions == 4:
        ...
        if tensor.shape[-2] == 1:
            tensor = tensor[..., 0, :]
            if isinstance(tensor, torch.Tensor):
                tensor = tensor.permute(3, 0, 1, 2)  # (W, H, D, C)
            elif isinstance(tensor, lazy_ops.lazy_loading.DatasetView):
                tensor = tensor.lazy_transpose([3, 0, 1, 2])  # (W, H, D, C)
        else:
            raise ValueError('5D is not supported for shape[-2] > 1')
    elif num_dimensions == 2:  # assume 2D monochannel (W, H)
        tensor = tensor.view((1,)+tensor.shape+(1,))  # (1, W, H, 1)
    elif num_dimensions == 3:  # 2D multichannel or 3D monochannel?
        if num_spatial_dims == 2:
            tensor = tensor.view(tensor.shape+(1,))  # (C, W, H, 1)
        elif num_spatial_dims == 3:  # (W, H, D)
            tensor = tensor.view((1,)+tensor.shape)  # (1, W, H, D)
        else:  # try to guess
            shape = tensor.shape
            ...
            if maybe_rgb:
                if shape[-1] == 3:  # (W, H, 3)
                    if isinstance(tensor, torch.Tensor):
                        ...
                        tensor = tensor.lazy_transpose([2, 0, 1])  # (3, W, H)
                tensor = tensor.view(tensor.shape+(1,))  # (3, W, H, 1)
            else:  # (W, H, D)
                tensor = tensor.view((1,)+tensor.shape)  # (1, W, H, D)
    else:
        message = (
